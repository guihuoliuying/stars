...
function Vector3:SqrMagnitudeWithoutY()
...
function Vector3.fixedCross(lhs, rhs, instance)
	instance.x = lhs.y * rhs.z - lhs.z * rhs.y
	instance.y = lhs.z * rhs.x - lhs.x * rhs.z
	instance.z = lhs.x * rhs.y - lhs.y * rhs.x
	return instance
end
...
local tempVector = Vector3.New(0,0,0)
local tmpQuter = nil
function Vector3.LocalOffToWorld( normalPos, normalRot, skewing )
	-- body
	if skewing == zeroVector then
		return normalPos:Clone()
	end
	-- 偏移朝向
	tempVector:Set(skewing.x, skewing.y, skewing.z)
	tempVector:SetNormalize()
	local skewingDir = Quaternion.LookRotation( tempVector )
	if skewingDir == nil then
		return normalPos:Clone()
	end
	tmpQuter = tmpQuter or Quaternion.identity
	tmpQuter:Set(normalRot.x, normalRot.y, normalRot.z, normalRot.w)--默认初始朝向
	if normalRot.x == 0 and normalRot.y == 0 and normalRot.z == 0 and normalRot.w == 1 then
		tmpQuter:Set(skewingDir.x, skewingDir.y, skewingDir.z, skewingDir.w)--初始角度为0时则直接使用偏移角度
	else
		if skewingDir.x ~= 0 or skewingDir.y ~= 0 or skewingDir.z ~= 0 or skewingDir.w ~= 1 then
			tmpQuter:Set(skewingDir.x, skewingDir.y, skewingDir.z, skewingDir.w)
			tmpQuter:MulQua( normalRot )
		end
	end
	local dis = Vector3.Distance(zeroVector, skewing)
	tempVector:Set(0,0,dis)
	return normalPos + tmpQuter * tempVector
end
...
Vector3.__eq = function(a,b)
	return ((a.x-b.x)*(a.x-b.x) + (a.y-b.y)*(a.y-b.y) + (a.z-b.z)*(a.z-b.z)) < 1e-10
end
...
--旋转并向前移动距离;
--原始的rotation. transform.rotation
-- function Vector3.rotateAndMoveForward(srcRotation, rotateX, rotateY, rotateZ, moveDistance)
-- 	local tmpQuter = Quaternion.Euler(rotateX, rotateY, rotateZ)*srcRotation;
-- 	local newPos = tmpQuter*(Vector3.forward*moveDistance);
-- 	return newPos;
-- end

function Vector3.NewByStr(str)
    local v = {x = 0, y = 0, z = 0}
    setmetatable(v, Vector3)
    local paraTable = StringUtils.split(str,"+")
    if paraTable == nil or #paraTable <= 0 then 
        return v
    else
        v.x = tonumber(paraTable[1]) or 0 
        v.y = tonumber(paraTable[2]) or 0
        v.z = tonumber(paraTable[3]) or 0 
        return v
    end 
end

...
-- 调整角度使变化幅度为最小
-- function Quaternion.ajustAngle( angelValue )
-- 	-- body
-- 	local temp = angelValue
-- 	if temp > 180 then
-- 		temp = temp - 360
...
function Quaternion.Quad( quaObj )
	-- body
	local quat = Quaternion.New()
	quat:setQuad( quaObj[1],quaObj[2],quaObj[3],quaObj[4])
	return quat
end

-- 战斗朝向为X，与unity默认朝向Z不一致，主要用于修正
-- function Quaternion.QuadWith90( quaObj )
-- 	-- body
-- 	local quat = Quaternion.New()
-- 	quat:setQuadWith90( quaObj[1],quaObj[2],quaObj[3],quaObj[4])
-- 	return quat
-- end
...
local function getHalfDegreeSinAndCos( sina, cosa )
	-- body
	local sinb, cosb
	sinb = sqrt( (1 - cosa )/2)
	cosb = sqrt( (1 + cosa )/2 )
...
local function judgeValue1( val )
	if val > 1 then
		val = 1
	end
	if val < -1 then
...
function Quaternion:setQuad( sina,cosa, sinb, cosb )
	sina,cosa, sinb, cosb = judgeValue1( sina ), judgeValue1( cosa ), judgeValue1( sinb ), judgeValue1( cosb )
	local sinZ,cosZ = getHalfDegreeSinAndCos( sinb, cosb)
	local sinY,cosY = getHalfDegreeSinAndCos(-sina, cosa)
...
-- function Quaternion:setQuadWith90( sina,cosa, sinb, cosb )
-- 	sina,cosa, sinb, cosb = judgeValue1( sina ), judgeValue1( cosa ), judgeValue1( sinb ), judgeValue1( cosb )
-- 	local sinX,cosX = getHalfDegreeSinAndCos( -sinb, cosb)
-- 	local sinY,cosY = getHalfDegreeSinAndCos(cosa, sina)	
...
function Quaternion.LookRotation(forward, up)
	...
	tempRight = tempRight or Vector3.zero
	tempUp = tempUp or Vector3.zero
	forward = forward / mag
	up = up or _up				
	tempRight = Vector3.fixedCross(up, forward, tempRight)   
	tempRight:SetNormalize()
    tempUp = Vector3.fixedCross(forward, tempRight, tempUp)	
    tempRight = Vector3.fixedCross(tempUp, forward, tempRight)
    local right = tempRight
    up = tempUp
...
--[[	local quat = _new(0,0,0,1)
	...
	if t > 0 then		
		...
		tempRot[1] = {right.x, up.x, forward.x}
		tempRot[2] = {right.y, up.y, forward.y}
		tempRot[3] = {right.z, up.z, forward.z}
		...
		local t = tempRot[i][i] - tempRot[j][j] - tempRot[k][k] + 1
		local s = 0.5 / sqrt(t)
		tempQ[i] = s * t
		local w = (tempRot[k][j] - tempRot[j][k]) * s
		tempQ[j] = (tempRot[j][i] + tempRot[i][j]) * s
		tempQ[k] = (tempRot[k][i] + tempRot[i][k]) * s
		
		local ret = _new(tempQ[1], tempQ[2], tempQ[3], w)			
		ret:SetNormalize()
		return ret
	end
end
...
function Quaternion:Forward2Vec(toVec)
	return self:MulVec3(_forward, toVec)
end

function Quaternion.MulVec3(self, point, vec)
	vec = vec or Vector3.New()
...
function Quaternion:MulQua( rhs )
	-- body
	local newX = (((self.w * rhs.x) + (self.x * rhs.w)) + (self.z * rhs.y)) - (self.y * rhs.z)
	local newY = (((self.w * rhs.y) + (self.y * rhs.w)) + (self.x * rhs.z)) - (self.z * rhs.x)
	local newZ = (((self.w * rhs.z) + (self.z * rhs.w)) + (self.y * rhs.x)) - (self.x * rhs.y)
	local newW = (((self.w * rhs.w) - (self.x * rhs.x)) - (self.y * rhs.y)) - (self.z * rhs.z)
	self:Set( newX, newY, newZ, newW )
end

--y轴旋转
-- function Quaternion:RotateY( sinY, cosY )
-- 	-- body
-- 	self.x = self.x * cosY - self.z * sinY
-- 	self.y = self.y * cosY + self.w * sinY
-- 	self.z = self.z * cosY + self.x * sinY
-- 	self.w = self.w * cosY - self.y * sinY
-- end

Quaternion.__mul = function(lhs, rhs)
	if Quaternion == getmetatable(rhs) then
		return Quaternion.New((((lhs.w * rhs.x) + (lhs.x * rhs.w)) + (lhs.z * rhs.y)) - (lhs.y * rhs.z), 

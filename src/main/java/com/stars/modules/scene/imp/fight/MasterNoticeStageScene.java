 ...
 */
public class MasterNoticeStageScene extends FightScene {
    @Override
    public boolean canEnter(Map<String, Module> moduleMap, Object stageId) {
        ...
        //判断是否存在已接受的皇榜任务，类型是通关的且关卡id为该关卡的id
       	MasterNoticeModule masterNoticeModule = (MasterNoticeModule)moduleMap.get(MConst.MasterNotice);
       	Map<Integer, MasterNoticeData> masterNotices = masterNoticeModule.getMasterNoticeDatas();
       	if (masterNotices != null) {
       		for (MasterNoticeData data : masterNotices.values()) {
				if (data.getStatus() == MasterNoticeData.STATUS_ACCEPT) {
					...
					if (vo != null && vo.getTaskType() == MasterNoticeVo.TASK_TYPE_PASS_STAGE && vo.getTaskParam() == tmpStageId) {
						return true;
					}
				}
			}
       	...
       	//PacketManager.send(masterNoticeModule.id(), new ClientText("masternotice_task_not_exist"));
       	LogUtil.info("MasterNoticeStageScene.canEnter masternotice task not exist,stageId=" + stageId);
        return false;
    }
    ...
    public void enter(Map<String, Module> moduleMap, Object stageId) {
        ...
        int count = dailyModule.getDailyCount(DailyManager.DAILYID_MASTER_NOTICE) + 1;
        ServerLogModule serverLogModule = (ServerLogModule)moduleMap.get(MConst.ServerLog);
        serverLogModule.Log_core_activity(ThemeType.ACTIVITY_21.getOperateId(), ThemeType.ACTIVITY_START.getOperateName(),
        		ThemeType.ACTIVITY_21.getThemeId(), serverLogModule.makeJuci(), ThemeType.ACTIVITY_21.getThemeId(),
    ...
    public void exit(Map<String, Module> moduleMap) {
        if (stageStatus == SceneManager.STAGE_PROCEEDING || stageStatus == SceneManager.STAGE_PAUSE) {
            ...
            toolModule.addAndSend(totalDropMap, EventType.MASTERNOTICE.getCode());
        }
    }
    ...
    public void finishDeal(Map<String, Module> moduleMap, byte finish) {
				...
				for (MasterNoticeData data : masterNotices.values()) {
					if (data.getStatus() == MasterNoticeData.STATUS_ACCEPT) {
						...
						if (vo != null && vo.getTaskType() == MasterNoticeVo.TASK_TYPE_PASS_STAGE && vo.getTaskParam() == stageId) {
							MapUtil.add(rewardMap, vo.getAwardMap());
						}
					}
				}
			}
        ...
        serverLogModule.Log_core_activity(ThemeType.ACTIVITY_21.getOperateId(), result.getOperateName(),
        		ThemeType.ACTIVITY_21.getThemeId(), serverLogModule.makeJuci(), ThemeType.ACTIVITY_21.getThemeId(),
                Integer.toString(this.stageId), passTimeSecondStr);
        ToolModule toolModule = (ToolModule) moduleMap.get(MConst.Tool);
        Map<Integer,Integer> map = toolModule.addAndSend(rewardMap, EventType.MASTERNOTICE.getCode());
        ...
        SceneModule sceneModule = (SceneModule) moduleMap.get(MConst.Scene);
        ClientStageFinish clientStageFinish = new ClientStageFinish(SceneManager.SCENETYPE_MASTER_NOTICE_STAGE, finish);
        ...
        if (finish == SceneManager.STAGE_VICTORY) {
            sceneModule.dispatchEvent(new PassMasterNoticeStageEvent(this.stageId));
        }
    }
}

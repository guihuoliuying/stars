 ...
 */
public class EscortTeamHandler implements TeamHandler {
    @Override
    public void createTeam(Map<String, Module> moduleMap, BaseTeamMember creator, byte teamType, int target) {
        if(creator==null) return;
        String checkResult = checkEscortCondition(moduleMap,true);
        if(checkResult!=null){
     ...
     *  组队运镖条件检测
     */
    private String checkEscortCondition(Map<String, Module> moduleMap,boolean checkRemainTimes){
        ...
        if(checkRemainTimes) {
            ...
            if (escortModule.getRoleEscort().getRemainTime() <= 0) {
    ...
    public boolean selfJoinInTeam(Map<String, Module> moduleMap, long roleId, int teamTarget, BaseTeam team) {
        String checkResult = checkEscortCondition(moduleMap,false);
        if(checkResult!=null){
            if(!checkResult.equals("")){
    ...
    public boolean otherJoinInTeam(long target, int teamTarget) {
        return ServiceHelper.escortService().hasJoinEscortTeamPermit(target);
    }
    ...
    public void handleOpenTeamUI(long roleId,Map<String, Module> moduleMap) {
        String conditionCheck = checkEscortCondition(moduleMap,false);
        if(conditionCheck!=null) return;//不满足要求

        ServiceHelper.escortService().addToEscortPermitSet(roleId);
    }

 ...
 */
public class OpActFamilyFightScoreActor extends ServiceActor implements OpActFamilyFightScore {
    private int curActivityId = -1;
    private boolean isStarting = false;

    @Override
    public void init() throws Throwable {
        ServiceSystem.getOrAdd(SConst.OpActFamilyFightSocre, this);
        curActivityId = OperateActivityManager.getFirstActIdbyActType(OperateActivityConstant.ActType_NewServerFamilyFightScore);
    ...
    public void view(long roleId) {
        ...
        ClientOpActFamilyFightScore act = new ClientOpActFamilyFightScore();
    ...
    public void checkActivityFlowState(int activityId) {
        ...
        List<OperateActVo> operateActVos = OperateActivityManager.getOperateActVoListByType(OperateActivityConstant.ActType_NewServerFamilyFightScore);
    ...
    private void sendReward(int activityId) {
        ...
        if (rankType == NewServerRankConstant.TYPE_FAMILY_FIGHT_SCORE) {
            int maxRank = NewServerRankManager.getMaxRewardRank(activityId);
            List<AbstractRankPo> rankPoList = ServiceHelper.rankService().getFrontRank(RankConstant.RANKID_FAMILYFIGHTSCORE, maxRank);
            ...
            if (activityRewadList != null) {
                ...
                for (NewServerRankVo vo : activityRewadList) {
                    ...
                    for (int rank = rankStart; rank <= rankEnd; rank++) {
                        ...
                        FamilyRankPo rankPo = (FamilyRankPo) rankPoList.get(index);
                        if (rankPo != null) {
                            Map<Byte, Integer> dropMap = new HashMap<>(vo.getFamilyFightScoreRankMap());
                            ServiceHelper.familyMainService().sendFamilySocreOpActAwardEmail(rankPo.getFamilyId(), dropMap, rank);
                        }
                    }
                }
            }
